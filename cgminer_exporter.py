...
class MetricsHandler(tornado.web.RequestHandler):
  def get(self):
    ...
    f.write(json.dumps(metricdata))
    #print metricdata['devs']
    if 'CGMiner' in metricdata['version']['VERSION'][0]:
      ...
      try: 
        # Antminers 
        miner = metricdata['version']['VERSION'][0]['Miner'] 
        mtype = metricdata['version']['VERSION'][0]['Type'] 
        mver  = metricdata['version']['VERSION'][0]['CGMiner'] 
        mapi  = metricdata['version']['VERSION'][0]['API'] 
        found = True
      except KeyError, e:
        found = False
      tags = 'instance="%s",cgminer_version="%s",api_version="%s",type="%s",miner="%s"'%(target,mver,mapi,mtype,miner)
      try: 
        # DT1 
        if not found:
          miner = metricdata['version']['VERSION'][0]['CGMiner'] 
          mtype = metricdata['devs']['DEVS'][0]['Name'] 
          mver  = metricdata['version']['VERSION'][0]['CGMiner'] 
          mapi  = metricdata['version']['VERSION'][0]['API'] 
          found = True
      except KeyError, e: 
...
def metric_summary(data, tags):
  ...
  try:
    ...
    pass
  try:
    string += 'cgminer_summary_ghs_average{%s} %s\n'%(localtags, data['SUMMARY'][0]['MHS av']/1000)
    string += 'cgminer_summary_ghs_5s{%s} %s\n'%(localtags, data['SUMMARY'][0]['MHS 5s']/1000)
  except KeyError, e:
    pass
